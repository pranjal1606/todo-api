...
export const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    if (!process.env.JWT_ACCESS_SECRET || process.env.JWT_ACCESS_SECRET.length < 32) {
      return next(new AppError("Server configuration error", StatusCodes.INTERNAL_SERVER_ERROR));
    }
    ...
    if (!user || user.deletedAt) {
      return next(new AppError("Unauthorized - User not found or deleted", StatusCodes.UNAUTHORIZED));
    }
    ...
    return next(new AppError("Unauthorized - Invalid or expired token", StatusCodes.UNAUTHORIZED));
  }
};
